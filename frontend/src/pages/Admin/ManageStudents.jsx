...
import { Users, UserPlus, Search, X, Edit2, Trash2 } from 'lucide-react';
...
const ManageStudents = () => {
    ...
    const closeModal = () => {
        ...
        setFormData({ name: '', rollNumber: '', department: '', currentSemester: '1', username: '', password: '', admissionStatus: 'ADMITTED' });
    };
                        ...
                        <tbody className="divide-y divide-surface-100">
                            {filteredStudents.map((student) => (
                                <tr key={student.id} className="hover:bg-surface-50 transition-colors">
                                    ...
                                    <td className="py-4 px-6">
                                        <span className={`flex items-center gap-2 text-sm font-medium ${student.admissionStatus === 'ADMITTED' ? 'text-emerald-600' : 'text-amber-600'}`}>
                                            <div className={`w-2 h-2 rounded-full ${student.admissionStatus === 'ADMITTED' ? 'bg-emerald-500' : 'bg-amber-500'}`}></div>
                                            {student.admissionStatus}
                                        </span>
                                    </td>
                                    <td className="py-4 px-6 text-right">
                                        <div className="flex justify-end gap-2">
                                            <button onClick={() => openEditModal(student)} className="p-2 text-primary-600 hover:bg-primary-50 rounded-lg transition-colors">
                                                <Edit2 size={18} />
                                            </button>
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                        ...
                        >
                            <div className="flex justify-between items-center mb-6">
                                ...
                                <button type="button" onClick={closeModal} className="p-2 hover:bg-surface-100 rounded-full transition-colors">
                                    <X size={20} />
                                </button>
                            </div>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <input required disabled={!!editingStudent} name="rollNumber" className="w-full p-3 rounded-xl bg-surface-50 border border-surface-200 outline-none focus:ring-2 focus:ring-primary-500 disabled:opacity-50" value={formData.rollNumber} onChange={(e) => setFormData({ ...formData, rollNumber: e.target.value })} />
                                    </div>
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-1">
                                ...
                                {editingStudent && (
                                    <div className="space-y-1">
                                        ...
                                        <select className="w-full p-3 rounded-xl bg-surface-50 border border-surface-200 outline-none focus:ring-2 focus:ring-primary-500" value={formData.admissionStatus} onChange={(e) => setFormData({ ...formData, admissionStatus: e.target.value })}>
                                ...
                                <button type="submit" className="w-full btn-primary py-4 rounded-xl mt-4">{editingStudent ? 'Save Changes' : 'Confirm Registration'}</button>
                            </form>
                        </motion.div>
                    </div>
                )}
