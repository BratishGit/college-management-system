        ...
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            {user.role === 'STUDENT' ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {subjects.map(sub => {
                        const subRecords = records.filter(r => r.course?.id === sub.id);
                    ...
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-surface-100">
                            {records.filter(r =>
                                ...
                                r.course?.name?.toLowerCase().includes(filter.toLowerCase())
                            ).map((r, i) => (
                                <tr key={i} className="hover:bg-surface-50 transition-colors">
                                    ...
                                    <td className="py-4 px-6">
                                        <p className="text-sm font-semibold">{r.course?.name}</p>
                                        <p className="text-[10px] text-surface-400 font-mono">{r.course?.code}</p>
                                    </td>
