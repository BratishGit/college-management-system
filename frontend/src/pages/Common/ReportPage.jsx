...
const ReportPage = ({ user }) => {
    ...
    const fetchData = async () => {
        try {
            // Simulated fetch since reports backend is not yet implemented
            setReports([]);
            ...
            if (user.role === 'TEACHER') {
                try {
                    ...
                    console.log("Could not fetch students for reports");
                }
            }
        } catch (err) {
            console.error(err);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        alert('Reporting module is currently under maintenance. Please contact admin directly.');
        setModalOpen(false);
